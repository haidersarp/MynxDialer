...
// Internal team chat (agent) — floating launcher + slide-up panel.
// Tabs: Announce (admin broadcast, default), <Campaign> (this agent's campaign
// group chat), Direct (1:1 DM). Supports image/doc attachments and admin
// "priority" messages that pop up full-screen. Rides the existing socket; never
// touches calls/Asterisk.
...
export default function ChatWidget({ user: userProp, campaignId, campaignName }) {
  ...
  const [pendingAtt, setPendingAtt] = useState(null);  // {url,name,type}
  const [attaching, setAttaching] = useState(false);
  const [popups, setPopups] = useState([]);            // priority messages
  ...
  // Louder two-tone "ding-dong" via Web Audio (no asset). resume() satisfies autoplay policy.
  const beep = useCallback(() => {
    try {
      ...
      tone(784, 0, 0.18, 0.32);      // G5
      tone(1047, 0.12, 0.24, 0.32);  // C6 — clearly audible
    } catch (_) {}
  }, []);
  ...
  useEffect(() => {
    ...
    const onMsg = (m) => {
      ...
      if (m.sender_id !== myId) {
        ...
        if (m.priority) setPopups(p => [...p, m]);     // priority → always pop up
  ...
  const loadHistory = useCallback(async () => {
    try {
      ...
      if (tab === 'campaign') { if (!campaignId) { setMessages([]); return; } params.campaign_id = campaignId; }
      const data = await api.get('/chat/history', { params });
      setMessages(Array.isArray(data) ? data : []);
      scrollDown();
    } catch (_) { setMessages([]); }
  }, [tab, dmUser, campaignId]);

  const loadUsers = useCallback(async () => {
    try { const data = await api.get('/chat/users'); setUsers(Array.isArray(data) ? data : []); } catch (_) {}
  }, []);

  useEffect(() => { if (open && isAuthed) loadHistory(); }, [open, tab, dmUser, isAuthed, loadHistory]);
  useEffect(() => { if (open && isAuthed) { loadUsers(); setUnread(0); } }, [open, isAuthed, loadUsers]);
  ...
  const send = () => {
    ...
    if (tab === 'broadcast' && !isAdmin) return;
    if (tab === 'campaign' && !campaignId) return;
    getSocket().emit('chat:send', {
      channel: tab,
      recipient_id: tab === 'dm' ? dmUser.id : undefined,
      campaign_id: tab === 'campaign' ? campaignId : undefined,
      body,
  ...
  // Open the conversation a priority message belongs to (so the agent lands right
  // on it in the thread, red-marked) and dismiss that popup.
  const openInChat = (p) => {
    setOpen(true);
    if (p.channel === 'dm') { setTab('dm'); setDmUser({ id: p.sender_id, name: p.sender_name, role: p.sender_role }); }
    else if (p.channel === 'campaign') { setTab('campaign'); }
    else { setTab('broadcast'); }
  ...
  const tabs = [
    ...
    { key: 'campaign', label: campaignName || 'Campaign' },
    { key: 'dm', label: 'Direct' }
  ];
  const canType = (tab === 'broadcast' && isAdmin) || (tab === 'campaign' && !!campaignId) || (tab === 'dm' && !!dmUser);

  return (
    <>
      <style>{`@keyframes chatShake{0%,100%{transform:translateX(0) rotate(0)}15%{transform:translateX(-3px) rotate(-7deg)}30%{transform:translateX(3px) rotate(7deg)}45%{transform:translateX(-3px) rotate(-5deg)}60%{transform:translateX(3px) rotate(5deg)}75%{transform:translateX(-2px) rotate(-2deg)}}@keyframes chatBadgePop{0%{transform:scale(0.4)}60%{transform:scale(1.25)}100%{transform:scale(1)}}@keyframes chatPopIn{from{transform:translateY(12px) scale(.96);opacity:0}to{transform:none;opacity:1}}`}</style>

      {/* Priority popups (always visible until closed) */}
      {popups.length > 0 && (
        <div style={{ position: 'fixed', right: 20, bottom: 86, zIndex: 10000, display: 'flex', flexDirection: 'column', gap: 10, maxWidth: 340 }}>
          {popups.map(p => (
            <div key={p.id} style={{ width: 330, background: '#fff', borderRadius: 14, overflow: 'hidden', boxShadow: '0 22px 60px rgba(0,0,0,0.45)', border: '2px solid #ef4444', animation: 'chatPopIn 0.25s ease' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '10px 12px', background: 'linear-gradient(135deg,#ef4444,#f97316)', color: '#fff' }}>
                <span style={{ fontSize: 15 }}>⚠</span>
                <div style={{ flex: 1, fontWeight: 800, fontSize: 12 }}>Priority — {p.sender_name} <span style={{ opacity: 0.85, fontWeight: 600 }}>· {p.channel === 'dm' ? 'Direct' : p.channel === 'campaign' ? 'Campaign' : 'Announce'}</span></div>
                <button onClick={() => setPopups(list => list.filter(x => x.id !== p.id))} style={{ background: 'rgba(255,255,255,0.25)', border: 'none', borderRadius: 8, width: 24, height: 24, color: '#fff', cursor: 'pointer', fontSize: 15, lineHeight: 1 }}>×</button>
              </div>
              <div onClick={() => openInChat(p)} style={{ padding: '12px 14px', color: '#1f2937', fontSize: 14, lineHeight: 1.45, cursor: 'pointer' }}>
                {p.body}
                {p.attachment_url && p.attachment_type === 'image' && (
                  <a href={p.attachment_url} target="_blank" rel="noreferrer" onClick={e => e.stopPropagation()}><img src={p.attachment_url} alt={p.attachment_name} style={{ maxWidth: '100%', borderRadius: 8, marginTop: 8, display: 'block' }} /></a>
                )}
                {p.attachment_url && p.attachment_type !== 'image' && (
                  <a href={p.attachment_url} target="_blank" rel="noreferrer" onClick={e => e.stopPropagation()} style={{ display: 'inline-block', marginTop: 8, color: '#2563eb', fontWeight: 700 }}>📎 {p.attachment_name}</a>
                )}
              </div>
              <button onClick={() => openInChat(p)} style={{ width: '100%', border: 'none', borderTop: '1px solid #fee2e2', background: '#fff5f5', color: '#dc2626', fontWeight: 800, fontSize: 12, padding: '9px 0', cursor: 'pointer' }}>Open in chat ›</button>
            </div>
          ))}
        </div>
      )}
      ...
      <button onClick={() => setOpen(o => !o)} title="Team chat" style={{
        ...
        {unread > 0 && (
          <span key={unread} style={{
            ...
            display: 'flex', alignItems: 'center', justifyContent: 'center', border: '2px solid #0f1622',
      ...
      {open && (
        <div style={{
          position: 'fixed', right: 20, bottom: 86, zIndex: 9990,
          width: 350, height: 470, maxHeight: 'calc(100vh - 120px)',
          background: PANEL_BG, border: `1px solid ${BORDER}`, borderRadius: 16,
          boxShadow: '0 20px 50px rgba(0,0,0,0.5)', display: 'flex', flexDirection: 'column', overflow: 'hidden'
        }}>
          {/* Header */}
          <div style={{ padding: '12px 14px', borderBottom: `1px solid ${BORDER}`, display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ color: TEXT2, fontSize: 10, marginTop: 2 }}>MynxDialer</div>
            </div>
            <button onClick={toggleSound} title={soundOn ? 'Message sound ON — click to mute' : 'Message sound OFF — click to enable'}
              style={{ display: 'flex', alignItems: 'center', gap: 4, background: 'none', border: `1px solid ${BORDER}`, borderRadius: 20, padding: '3px 9px', color: soundOn ? ACCENT : TEXT2, cursor: 'pointer', fontSize: 10, fontWeight: 700, whiteSpace: 'nowrap' }}>
          ...
          <div style={{ display: 'flex', padding: '8px 8px 0', gap: 4 }}>
            {tabs.map(t => (
              <button key={t.key} onClick={() => { setTab(t.key); if (t.key !== 'dm') setDmUser(null); }} style={{
                flex: 1, padding: '7px 4px', borderRadius: 8, border: 'none', cursor: 'pointer', fontSize: 11.5, fontWeight: 700,
                background: tab === t.key ? CARD_BG : 'transparent', color: tab === t.key ? ACCENT : TEXT2,
                borderBottom: tab === t.key ? `2px solid ${ACCENT}` : '2px solid transparent', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis'
              }}>{t.label}</button>
            ))}
          </div>
          ...
          {tab === 'dm' && !dmUser ? (
            ...
            <>
              ...
              <div ref={bodyRef} onPaste={onPaste} style={{ flex: 1, overflowY: 'auto', padding: 12, display: 'flex', flexDirection: 'column', gap: 8 }}>
                {messages.length === 0 && (
                  <div style={{ color: TEXT2, fontSize: 12, textAlign: 'center', margin: 'auto', padding: 20 }}>
                    {tab === 'broadcast' ? 'No announcements yet.' : tab === 'campaign' ? (campaignName ? `${campaignName} team chat` : 'Campaign chat') : 'No messages yet.'}
                  </div>
                )}
              ...
              {pendingAtt && (
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '6px 10px', borderTop: `1px solid ${BORDER}`, background: CARD_BG }}>
                  {pendingAtt.type === 'image'
                    ? <img src={pendingAtt.url} alt="" style={{ width: 36, height: 36, objectFit: 'cover', borderRadius: 6 }} />
              ...
              {canType ? (
                <div style={{ padding: 10, borderTop: pendingAtt ? 'none' : `1px solid ${BORDER}`, display: 'flex', gap: 8, alignItems: 'center' }}>
                  ...
                  <button onClick={() => fileRef.current && fileRef.current.click()} title="Attach a file" disabled={attaching}
                    style={{ background: CARD_BG, border: `1px solid ${BORDER}`, borderRadius: 10, width: 38, height: 38, color: ACCENT, cursor: 'pointer', fontSize: 16, flexShrink: 0 }}>
                    {attaching ? '…' : '📎'}
                  </button>
                  <input value={text} onChange={e => setText(e.target.value)} onPaste={onPaste}
                    onKeyDown={e => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); send(); } }}
                    placeholder="Type a message… (paste an image too)"
                    style={{ flex: 1, background: CARD_BG, border: `1px solid ${BORDER}`, borderRadius: 10, padding: '9px 12px', color: TEXT, fontSize: 13, outline: 'none' }} />
                  <button onClick={send} style={{ background: `linear-gradient(135deg, ${ACCENT}, ${ACCENT2})`, border: 'none', borderRadius: 10, padding: '0 14px', height: 38, color: '#fff', fontWeight: 800, cursor: 'pointer', fontSize: 13, flexShrink: 0 }}>Send</button>
                </div>
              ) : (
                <div style={{ padding: '12px 14px', borderTop: `1px solid ${BORDER}`, color: TEXT2, fontSize: 11.5, textAlign: 'center' }}>
                  {tab === 'broadcast' ? "Only admins post announcements — you'll see them here." : 'Read-only.'}
                </div>
              )}
            </>
          )}
...
function Bubble({ m, mine, fmtTime }) {
  ...
  return (
    <div style={{ alignSelf: mine ? 'flex-end' : 'flex-start', maxWidth: '82%' }}>
      ...
      <div style={{
        background: mine ? `linear-gradient(135deg, ${ACCENT2}, #5b21b6)` : CARD_BG,
        color: mine ? '#fff' : TEXT,
        border: prio ? '2px solid #ef4444' : (mine ? 'none' : `1px solid ${BORDER}`),
        boxShadow: prio ? '0 0 12px rgba(239,68,68,0.35)' : 'none',
        padding: '8px 11px', borderRadius: 12, fontSize: 13, lineHeight: 1.35, wordBreak: 'break-word'
      }}>
        {m.body}
