...
let roomSession = null;    // the persistent *88 conference leg
let manualSession = null;  // a direct manual/outbound dial, separate from the room
let pendingRoom = false;   // set by makeCall() so newRTCSession can tag the new leg
let eventHandlers = {};
...
export function initSIP(config, handlers) {
  ...
  ua.on('newRTCSession', (data) => {
    ...
    // Tag this leg so the persistent room (*88) is never confused with a manual
    // dial or an incoming call. Only a locally-placed *88 call is the room.
    session._isRoom = (originator === 'local') ? pendingRoom : false;
    pendingRoom = false;
    if (session._isRoom) roomSession = session;
    else if (originator === 'local') manualSession = session;
    ...
    session.on('failed', (e) => {
      ...
      if (roomSession === session) roomSession = null;
      if (manualSession === session) manualSession = null;
      if (currentSession === session) currentSession = roomSession;
      handlers?.onCallFailed?.(e.cause, session);
    });
    ...
    session.on('ended', (e) => {
      ...
      if (manualSession === session) manualSession = null;
      if (currentSession === session) currentSession = roomSession;
      handlers?.onCallEnded?.(e.cause, session);
    });
...
export function makeCall(target, sipConfig) {
  ...
  pendingRoom = (target === '*88'); // tag the room leg for newRTCSession
...
// Hang up ONLY a direct manual/outbound dial, leaving the room (*88) leg intact.
// Used when an agent hangs up a manually-dialed call while sitting in their room.
export function hangupManual() {
  const s = manualSession;
  if (s) {
    try { s.terminate(); } catch (_) {}
    manualSession = null;
    if (currentSession === s) currentSession = roomSession;
  }
}
